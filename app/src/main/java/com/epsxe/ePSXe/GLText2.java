...
public class GLText2 {
    ...
    public GLText2(GL10 gl, AssetManager assets, int mPro, int mpadPos, int mpadTex, int mpadFrm, int mpadAlp, float pAlpha) {
        this.f126gl = gl;
        this.assets = assets;
        this.batch = new SpriteBatch2(gl, 100, mPro, mpadPos, mpadTex, mpadFrm, mpadAlp, pAlpha);
        this.mProgram = mPro;
    }
    ...
    public boolean load(String file, int size, int padX, int padY) {
        ...
        if (osVersion >= 12) {
            bsize = bitmap.getByteCount();
        } else {
            bsize = bitmap.getRowBytes() * bitmap.getHeight();
        }
        ByteBuffer buffer = ByteBuffer.allocateDirect(bsize);
        bitmap.copyPixelsToBuffer(buffer);
        buffer.position(0);
        GLES20.glTexImage2D(3553, 0, 6409, this.textureSize, this.textureSize, 0, 6409, 5121, buffer);
    ...
    public void draw(String text, float x, float y, int w, int h) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            this.batch.drawSprite(x2 / w, y2 / h, chrWidth / w, chrHeight / h, this.charRgn[c]);
            x2 += (this.charWidths[c] + this.spaceX) * this.scaleX;
        }
    }

    public float drawC(String text, float x, float y, int w, int h) {
        float len = getLength(text);
        draw(text, x - (len / 2.0f), y - (getCharHeight() / 2.0f), w, h);
        return len;
    }

    public float drawCX(String text, float x, float y, int w, int h) {
        float len = getLength(text);
        draw(text, x - (len / 2.0f), y, w, h);
        return len;
    }

    public void drawCY(String text, float x, float y, int w, int h) {
        draw(text, x, y - (getCharHeight() / 2.0f), w, h);
    }
    ...
    public void drawTexture(int width, int height, int fw, int fh) {
        GLES20.glBindTexture(3553, this.textureId);
        GLES20.glUseProgram(this.mProgram);
        this.batch.beginBatch(this.textureId);
        this.batch.drawSprite((this.textureSize / 2f) / fw, (height - (this.textureSize / 2f)) / fh, (float)this.textureSize / fw, (float)this.textureSize / fh, this.textureRgn);
        this.batch.endBatch();
    }
}
